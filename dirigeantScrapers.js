...
async function personnelScraper(url) {
    ...
    for (let indexGlobal = 0; indexGlobal < clubsDetails.length; indexGlobal++) {
       ...
       const elementsDetails = await page.evaluate(() => {
           return Array.from(document.querySelectorAll("#staff .row .col-xl-4 .item-player .btn")).map(x => x.getAttribute("href"))
       })
       ...
       for (let index = 0; index < elementsDetails.length ; index++) {
               ...
               // Age
               const [age] = await page.$x('/html/body/div[2]/div[2]/section/div/div/div/div[1]/div/div[2]/ul/li[2]/span');
               const ageTxt = await age.getProperty('textContent');
               const ageRawTxt = await ageTxt.jsonValue();

               // Date de Naissance
               const [dateNaissance] = await page.$x('/html/body/div[2]/div[2]/section/div/div/div/div[1]/div/div[2]/ul/li[3]/span');
               ...
               // Lieu de Naissance
               const [lieuNaissance] = await page.$x('/html/body/div[2]/div[2]/section/div/div/div/div[1]/div/div[2]/ul/li[4]/span');
               ...
               // wilaya
               const [wilaya] = await page.$x('/html/body/div[2]/div[2]/section/div/div/div/div[1]/div/div[2]/ul/li[5]/span');
               ...
               console.log(table, ageRawTxt, dateNaissanceRawTxt, lieuNaissanceRawTxt, wilayaRawTxt)
           ...
           if (table.length === 4) {
               ...
               if (table.length === 5) {
                ...
                if (table.length === 6) {
                    worksheet.cell(index + 2, 8)
                    .string(table[2] + " " + table[3] + " " + table[4] + " " + table[5])
                }
                else {
                    if (table.length === 7) {
                        worksheet.cell(index + 2, 8)
                        .string(table[2] + " " + table[3] + " " + table[4] + " " + table[5] + " " + table[6])
                    }
                    else {
                        if (table.length === 8) {
                            worksheet.cell(index + 2, 8)
                            .string(table[2] + " " + table[3] + " " + table[4] + " " + table[5] + " " + table[6] + " " + table[7])
                        }
                        else {
                            worksheet.cell(index + 2, 8)
                            .string(table[2])
